...
if (isset($_POST['required'])) {
  try {
    $bouncer->name('required')->required('Field must be filled in.')->validate(true);
  } catch (Exception $e) {
  }
}
...
<head>
  <meta charset="utf-8">
  <title>Bouncer required validation</title>
        ...
        <div class="col-lg-4 col-md-6 col-sm-8">
          <div class="bg-white p-5 rounded border">
            ...
            <form method="POST">
              <div class="mb-3">
                <?php
                $requiredErrorClass = 0 === $bouncer->get('valid', 'required') ? ' is-invalid' : '';
                $requiredErrorMessage = $bouncer->errors(array('before' => '<div class="invalid-feedback d-block">', 'after' => '</div>'), 'required');
                ?>
                <input class="form-control form-control-lg<?php echo $requiredErrorClass; ?>" type="text" name="required" placeholder="Enter an any text" />
                <?php echo $requiredErrorMessage; ?>
              </div>
              <div class="d-grid">
